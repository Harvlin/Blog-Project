...
const PostForm: React.FC<PostFormProps> = ({
  ...
  const [categoryId, setCategoryId] = useState(initialPost?.categoryEntity?.id || '');
  ...
  useEffect(() => {
    if (initialPost && editor) {
      ...
      setCategoryId(initialPost.categoryEntity?.id);
  ...
  const validateForm = (): boolean => {
    ...
    if (!categoryId) {
      newErrors.categoryEntity = 'Category is required';
    }
        ...
        <CardBody className="space-y-4">
          ...
          <div className="space-y-2">
            <Select
              ...
              isInvalid={!!errors.categoryEntity}
              errorMessage={errors.categoryEntity}
              isRequired
            >
              {categories.map((cat) => (
